...
if "error_display" not in st.session_state:
    st.session_state.error_display = None
...
# 캘린더 출력
state = calendar(events=events, options={"headerToolbar": {"left": "prev,next today", "center": "title", "right": "resourceTimelineMonth"}, "initialView": "resourceTimelineMonth", "resources": resources, "editable": True, "selectable": True, "height": "auto"}, key="final_calendar_v7")

# 드래그 업데이트 (성공 시에만 rerun, 실패 시엔 에러 메시지 고정)
if state.get("eventDrop"):
    ...
    try:
        ...
        st.session_state.error_display = f"이동 실패: {str(e)}"
        st.rerun()

if st.session_state.error_display:
    st.error(st.session_state.error_display)
