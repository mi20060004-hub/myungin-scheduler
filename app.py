...
# 4. 캘린더 출력
state = calendar(events=events, options={"headerToolbar": {"left": "prev,next today", "center": "title", "right": "resourceTimelineMonth"}, "initialView": "resourceTimelineMonth", "resources": resources, "editable": True, "selectable": True, "height": "auto"}, key="calendar_v8")

# 5. 드래그 업데이트 (에러 발생 시 st.rerun() 하지 않고 에러 저장)
if state.get("eventDrop"):
    ...
    try:
        ...
        st.session_state.sticky_error = None # 성공 시 초기화
        st.rerun()
    except Exception as e:
        st.session_state.sticky_error = f"업데이트 실패! 이 에러를 복사해서 알려주세요: {str(e)}"
        st.rerun()

# 에러 메시지 출력 (버튼 누르기 전까지 절대 안 사라짐)
if st.session_state.sticky_error:
    st.error(st.session_state.sticky_error)
    if st.button("에러 확인 후 삭제"):
        st.session_state.sticky_error = None
        st.rerun()

# 6. 생산 계획 직접 등록 (삭제 안 함)
st.markdown("---")
